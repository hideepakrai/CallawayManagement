 ...
 useEffect(()=>{
  ...
  if(getUserOrder && getUserOrder.length>0){
    getUserOrder.map((item)=>{
     if(item.brand_id===4 && (item.status==="Complete" || item.status==="Completed" )){
       allComp.push(item)
     }
     if(item.brand_id===4 && (item.status!="Complete" && item.status!=="Completed" )){
       allPend.push(item)
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div  style={{backgroundColor:"#000"}} className={`card-header row m-0 border-0 py-5` } 
      >
        <div className='col-6'>
        <h3 className='card-title fw-bold text-white pt-4'>Ogio </h3>
        </div>
